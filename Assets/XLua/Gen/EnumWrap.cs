...
namespace XLua.CSObjectWrap
{
    ...
    public class KEngineLoaderModeWrap
    {
		public static void __Register(RealStatePtr L)
        {
		    ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);
		    Utils.BeginObjectRegister(typeof(KEngine.LoaderMode), L, translator, 0, 0, 0, 0);
			Utils.EndObjectRegister(typeof(KEngine.LoaderMode), L, translator, null, null, null, null, null);
			
			Utils.BeginClassRegister(typeof(KEngine.LoaderMode), L, null, 3, 0, 0);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Async", KEngine.LoaderMode.Async);
            
            Utils.RegisterObject(L, translator, Utils.CLS_IDX, "Sync", KEngine.LoaderMode.Sync);
            
			Utils.RegisterFunc(L, Utils.CLS_IDX, "__CastFrom", __CastFrom);
            
            Utils.EndClassRegister(typeof(KEngine.LoaderMode), L, translator);
        }
        ...
        static int __CastFrom(RealStatePtr L)
		{
            ...
            if (lua_type == LuaTypes.LUA_TNUMBER)
            {
                translator.PushKEngineLoaderMode(L, (KEngine.LoaderMode)LuaAPI.xlua_tointeger(L, 1));
            }
			
            else if(lua_type == LuaTypes.LUA_TSTRING)
            {
			    if (LuaAPI.xlua_is_eq_str(L, 1, "Async"))
                {
                    translator.PushKEngineLoaderMode(L, KEngine.LoaderMode.Async);
                }
				else if (LuaAPI.xlua_is_eq_str(L, 1, "Sync"))
                {
                    translator.PushKEngineLoaderMode(L, KEngine.LoaderMode.Sync);
                }
				else
                {
                    return LuaAPI.luaL_error(L, "invalid string for KEngine.LoaderMode!");
                }
            }
			
            else
            {
                return LuaAPI.luaL_error(L, "invalid lua type for KEngine.LoaderMode! Expect number or string, got + " + lua_type);
            }
